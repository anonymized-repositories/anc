...
def add_model_names(model_class, hf_model_ids):
    ...
    hf_model_ids = {
        ...
        for model_name, hf_model_id in zip(model_names, hf_model_ids)
    }
    return hf_model_ids
...
def encode_dataset(
    dataset, lang, tokenizer, model, batch_size, surgery_prediction_head=None
):
...
def encode_batch(
    ...
    for layer_num in range(num_layers):
        ...
        if encode_cls:
            ...
            out_dict[f"cls_{layer_num}"] = (
                sent_reps_curr_layer_cls.detach().cpu().numpy()
            )

        if encode_token1:
            sent_reps_curr_layer_token1 = enc_batch.hidden_states[layer_num][:, 1]
            out_dict[f"token1_{layer_num}"] = (
                sent_reps_curr_layer_token1.detach().cpu().numpy()
            )
    ...
    if surgery_prediction_head is not None:
        ...
        prediction_head_dict = {
            k: v.detach().cpu().numpy() for k, v in prediction_head_dict.items()
        }
...
def get_hf_model_ids(model_class):
    ...
    if model_class == "norm_1M":
        hf_model_ids = [
            f"{normdir}/scale_post/checkpoint-1000000",
    ...
    #     hf_model_ids = [f'{normdir}/scale_normformer/checkpoint-1000000'],
        ...
        hf_model_ids = [
            f"{normdir}/scale_zero/checkpoint-100000",
